...
__global__ void vcf(float2 *psi) {
	...
	if ( tid < TID_MAX ) {
		psi[tid].x = vc[0].x * spsi.x - vc[0].y * spsi.y;
		psi[tid].y = vc[0].x * spsi.y + vc[0].y * spsi.x;
	}
}
'''.replace('TID0',str(vx0)).replace('TID_MAX',str(vx0+vwidth))
print kernels
...
'''
kpsi[:] = np.fft.fft(psi)
psi[:] = np.fft.ifft(lc_sqrt * kpsi)
psi[vx0:vx0+vwidth] *= vc
'''
plan.execute(psi_gpu)
psi_gpu = psi_gpu * lc_sqrt_gpu
plan.execute(psi_gpu, inverse=True)
vcf(psi_gpu, block=(256,1,1), grid=(vwidth/256+1,1))
