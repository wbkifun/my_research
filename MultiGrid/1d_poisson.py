...
import scipy as sp
from scipy import pi, sin, linspace, ones
from scipy.sparse import spdiags, spidentity
...
f = pi**2 * (sin(pi * x) + 4**2 * sin(4 * pi * x) + 9**2 * sin(9 * pi * x))
A = spdiags([-ones(n), 2*ones(n), -ones(n)], [-1, 0, 1], n, n).todense()
print A
b = f * h**2
uc = b / A
"""
...
def twogrid(A, b, nu1, nu2, gamma, x0, level, t=0):
   '''
   ...
   n = length(b)
   if n < 4:
      x = b / A     # solve exactly at coarsest level
